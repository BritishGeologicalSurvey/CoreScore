...
class coreImageProcessor():
    ...
    def __init__(self,imageDirectory):
        ...
        print(self.maskDir)
        print(self.path)
        self.dicto_ = OrderedDict({"Box":[0,0],"Rock_Fragment":[1,1],"Paper":[2,2],"Core_Plug":[3,3],"Text":[4,4]})
    ...
    def getRandomPiece(self,shape_type,shapes,blank):
        ...
        randomPiece = (np.random.randint(0,self.getNumPiecesInBox(shape_type,shapes)))
        Onemask = self.genOneMask(blank,shapes[shape_type][randomPiece],
                                  (self.dicto_[shape_type][0],self.dicto_[shape_type][1]))
    ...
    def makeRandomImages(self):
        ...
        self.saveMask(self.path+"/train_masks/"+randomString+".",self.mask)
...
#        print(type(oneMask))
        ...
        oneMask,randomPiece = self.getRandomPiece('Rock_Fragment',shape,blank)
        if resize is True:
            shape = self.genShape(self.dicto_,fp)
...
#        print(mask)
    ...
    def processImage(self,fp):
        ...
        img = self.openImage(self.path+"/train/"+str(self.getImageNames(fp)[:-4])+"png")
        width = img.shape[1]
        height = img.shape[0]
        blank = self.makeBlankImage(width,height)
...
#        print(self.translateShape(shape,self.getMinShape(shape)))
        self.saveMask(self.path+"/train_masks/"+str(self.getImageNames(fp))[:-4],mask)
...
#                    print(key)
    ...
    def genOneMask(self,img,shape,values):
        ...
        try:
            ImageDraw.Draw(img).polygon(shape,outline=values[0],fill=values[1])
        except:
...
coreProcessor = coreImageProcessor("/home/ziad/Documents/Deep_Learning/unet/Unet_Core/data/")
